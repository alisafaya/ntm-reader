...
from utils.eval_all import update_evaluators, ALL
from utils.cluster import ClusterList
...
class Predictor:
    ...
    def evaluate(self, perf=False, inspect=False):
        logging.info(
            f"loss count {self.model.loss_count} and sampled loss count {self.model.sampled_loss_count}"
        )
        self.reset()
        self.predictions = []
        coref_predictions = {}
        ...
        for doc_id, document in eval_iterator:
            ...
            predicted_clusters = cluster_list.get_clusters(
                self.singleton_eval, condensed=True, print_clusters=False
            )
            update_evaluators(
                self.evaluators,
                document,
            ...
            coref_predictions[document["doc_key"]] = cluster_list.as_list(
                self.singleton_eval
            )
        ...
        logging.info(
            f"ca: {self.model.correct_attach} and wa: {self.model.wrong_attach}"
        )
        end_time = time.time()
        logging.info(
            f"Evaluation on {len(self.data)} documents [{avg_loss:.3f}] took {end_time - start_time:.1f} seconds"
        )
        eval_dict = [
            f"{key}: {evaluator.prf_str()}, ({evaluator.get_count()} docs)"
...
def predict_and_score(config, data_prefix):
    ...
    eval_data = util.load_data(
        config[f"{data_prefix}_path"],
        config.get("num_dev_examples"),
        tokenizer=incremental_model.tokenizer,
    )

    predictor = Predictor(incremental_model, eval_data, config["singleton_eval"])
    with torch.no_grad():
        predictor.evaluate(perf=False)  # False by default
