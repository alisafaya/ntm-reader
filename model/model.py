...
class NTMReader(torch.nn.Module):
	def __init__(self, config):
		...
		logging.info("Number of backbone parameters: {}".format(util.count_parameters(self.backbone)))
		logging.info("Number of other trainable parameters: {}".format(util.count_parameters(self) - util.count_parameters(self.backbone)))
	...
	def prepare_document(self, document):
		...
		tokenized = tokenizer(text_segments, return_tensors="pt", padding=True, truncation=True, max_length=64, return_offsets_mapping=True)
		...
		if 'antecedent_map' in document:
			...
			for c in clusters[x]:
				clusters_tokenized = []
				for s, e in c:
					t_s, t_e = c2t_map[s], (c2t_map[e-1] + 1) # span end index is exclusive
	...
	def forward(self, segment, model_data, entities, start_idx, metrics=True):
		...
		span_embeddings, mention_scores, top_spans = self.mention_encoder(encoder_repr, tokenized["attention_mask"])
