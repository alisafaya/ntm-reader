...
class Incremental(torch.nn.Module):
    ...
    def read_given_spans(self, spans, start_idx, num_words):
        ...
        starts = torch.tensor(
            [span[0] - start_idx for span in valid_spans], device=self.device
        )
        ends = torch.tensor(
            [span[1] - start_idx for span in valid_spans], device=self.device
        )
        return starts, ends
    ...
    def get_top_spans(self, sentence_map, doc_embs, start_idx, given_spans=None):
        ...
        if not self.use_gold_spans:
            starts, ends = self.get_candidate_offsets(
                start_idx, num_words, sentence_map
            )
            top_span_embs, scores, top_k_indices = self.span_scorer(
                starts, ends, doc_embs
            )
            ...
            if starts.shape[0] == 0:
                return []
            top_span_embs, scores, _ = self.span_scorer(
                starts, ends, doc_embs, use_gold_spans=True
            )
        return list(zip(top_span_embs, starts.cpu(), ends.cpu(), scores))

    def update_clusterlist(
        self, clusterlist, cluster, score, best_cluster_idx, new_memory=None
    ):
        # Retrieve cluster if already classified
        best_cluster_idx = clusterlist.span_to_cluster.get(
            (cluster.start, cluster.end), best_cluster_idx
        )
        if best_cluster_idx == -1 or (
            self.mention_classifier and cluster.score < self.threshold
        ):
            return

        # update memory of the clusters
        if new_memory is not None:
            for idx, c in enumerate(clusterlist):
                c.emb = new_memory[idx + 1]

        if best_cluster_idx == 0:
            if new_memory is not None:  # initialized a new cluster
                cluster.emb = new_memory[0]
            clusterlist.append(cluster)
        else:
            clusterlist.merge(int(best_cluster_idx), cluster, score=score)

    def resolve_local(
        self, clusters, sent_gen, gold_map, doc_embs, train=False, metrics=True
    ):
        total_loss = None
        cpu_loss = 0
        num_spans = 0
        for sent_idx, _, cluster_list in sent_gen:
            ...
            for cluster in cluster_list:
                ...
                gold_cluster_ids = clusters.get_cluster_ids(
                    gold_span, original=(cluster.first.start, cluster.first.end)
                )  # in range(0, num_clusters + 1)
                ...
                if self.mention_classifier:
                    p_mention = torch.log_softmax(
                        torch.cat([self.threshold, cluster.score.view(1)]), dim=0
                    )
                ...
                if do_cluster_scoring:
                    ...
                    if len(clusters) == 0:
                        self.update_clusterlist(clusters, cluster, cluster.score, 0)
                        cluster_loss = None
                    else:
                        ...
                        gold_cluster_id = (
                            len(clusters) + 1
                            if (gold_span is None and not self.mention_classifier)
                            else gold_cluster_id
                        )
                        (
                            p_score,
                            best_cluster_idx,
                            new_memory,
                            cluster_loss,
                        ) = self.pair_scorer.loss(
                            doc_embs,
                            cluster.emb,
                            cluster_embs,
                            gold_cluster_id,
                            joint_score=cluster.score
                            if not self.mention_classifier
                            else None,
                        )  # [1,1] score

                        if new_memory is not None:  # meaning we detected a mention.
                            self.update_clusterlist(
                                clusters, cluster, p_score, best_cluster_idx, new_memory
                            )
                ...
                total_loss = util.safe_add(
                    total_loss, util.safe_add(span_loss, cluster_loss)
                )
                ...
                if not train or util.get_cuda_memory_allocated() > self.memory_limit:
                    if train and total_loss is not None:
                        ...
                        # print("flushing mem...", len(clusters))
                        # clusters.detach_()
                    cpu_loss = float(
                        util.safe_add(cpu_loss, total_loss)
                    )  # float() needed to detach total_loss
                    total_loss = None
    ...
    def forward(
        ...
        if mask is not None:
            doc_embs = torch.index_select(
                doc_embs, 1, torch.tensor(mask).to(self.device)
            )
        ...
        if self.use_gold_spans:
            if "gold_spans" in model_data:
                ...
                # also dedup
                gold_spans = list(
                    {tuple(x) for x in util.flatten(model_data["clusters"])}
                )
        else:
            gold_spans = None
        top_spans = self.get_top_spans(
            model_data["sentence_map"], doc_embs, start_idx, given_spans=gold_spans
        )
        if len(top_spans) == 0:
            # Nothing to update
            return 0.0
        ...
        sent_gen = util.get_sentence_iter(
            sentences, segment_map, top_spans, start_idx, None, self.cluster
        )
