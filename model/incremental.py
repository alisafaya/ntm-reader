...
class Incremental(torch.nn.Module):
	def __init__(self, config):
		...
		(self.encoder, self.span_scorer, self.pair_scorer, self.device) = create_from_scratch(config)
	...
	def update_clusterlist(self, clusterlist, cluster, score, best_cluster_idx, new_memory=None):
		# Retrieve cluster if already classified
		best_cluster_idx = clusterlist.span_to_cluster.get((cluster.start, cluster.end), best_cluster_idx)
		if best_cluster_idx == -1 or (self.mention_classifier and cluster.score < self.threshold):
			return
	...
	def resolve_local(self, clusters, sent_gen, gold_map, doc_embs, train=False, metrics=True):
		total_loss = None
		cpu_loss = 0
		num_spans = 0
			...
			for cluster in cluster_list:
				...
				if do_cluster_scoring:
					...
					if len(clusters) == 0:
						...
						gold_cluster_id = len(clusters) + 1 if (gold_span is None and not self.mention_classifier) else gold_cluster_id
						p_score, best_cluster_idx, new_memory, cluster_loss = self.pair_scorer.loss(doc_embs, cluster.emb, cluster_embs, gold_cluster_id, 
																												joint_score=cluster.score if not self.mention_classifier else None)  # [1,1] score

						if new_memory is not None: # meaning we detected a mention.
							self.update_clusterlist(clusters, cluster, p_score, best_cluster_idx, new_memory)
				...
				total_loss = util.safe_add(total_loss, util.safe_add(span_loss, cluster_loss))
	...
	def forward(self, segment, model_data, clusters, start_idx, mask, train=False, metrics=True, consolidate=True):
		...
		if consolidate:
			...
			cons_loss = self.resolve_local(new_clusters, [(None, None, clusters)], model_data["antecedent_map"], None, train=train, metrics=False)
		...
		sent_gen = util.get_sentence_iter(sentences, segment_map, top_spans, start_idx, None, self.cluster)
		spans_loss = self.resolve_local(clusters, sent_gen, model_data["antecedent_map"], doc_embs.detach(), train=train, metrics=metrics)
	...
	def set_threshold(self, threshold):
		self.threshold = torch.nn.Parameter(data=torch.Tensor([threshold]).to(self.device), requires_grad=False)
