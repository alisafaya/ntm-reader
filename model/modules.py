...
class MentionEncoder(nn.Module):
    ...
    def forward(self, segment_embeddings, segment_mask, gold_spans=None):
        ...
        if gold_spans is not None:
            ...
            candidate_spans = enumerate_spans(
                torch.arange(num_words), offset=0, max_span_width=self.max_span_width
            )  # [num_candidates, 2]
            candidate_spans = sorted(
                candidate_spans[::-1], key=lambda x: x[0]
            )  # spans that end last come last.
            candidate_spans = torch.LongTensor([candidate_spans] * batch_size).to(
                segment_embeddings.device
            )

        # encode all possible spans.
        span_embeddings = self.span_encoder(
            segment_embeddings, candidate_spans, segment_mask
        )  # [batch_size, max_num_spans, span_encoder_dim]

        # project span representations.
        span_embeddings = self.span_encoder_mlp(
            span_embeddings
        )  # [batch_size, max_num_spans, hidden_size]

        # Get mention scores.
        mention_scores = self.mention_scorer(
            span_embeddings
        )  # [batch_size, max_num_spans, 1]
        ...
        if gold_spans is not None:
            ...
            i, j = torch.where(
                candidate_spans[:, :, -1] >= segment_mask.sum(-1).view(-1, 1)
            )
            ...
            _, top_k_indices = torch.topk(
                mention_scores.squeeze(), k, sorted=False
            )  # sorted=False to reduce overhead.

            if self.training:
                sampled_indices = (
                    torch.randperm(mention_scores.size(1))[:k]
                    .view(1, -1)
                    .repeat(batch_size, 1)
                    .to(top_k_indices.device)
                )
                top_k_indices = torch.cat([top_k_indices, sampled_indices], dim=1)

            top_k_indices, _ = torch.sort(
                top_k_indices, 1
            )  # to keep the sequential order of candidate spans.
            top_scores = nn_util.batched_index_select(
                mention_scores, top_k_indices
            ).squeeze()  # [batch_size, k, hidden_size]
            top_span_embs = nn_util.batched_index_select(
                span_embeddings, top_k_indices
            )  # [batch_size, k]
            top_spans = nn_util.batched_index_select(
                candidate_spans, top_k_indices
            )  # [batch_size, k, 2]
...
class NTMAdresser(nn.Module):
    ...
    def forward(self, query, keys, entity_size, mention_score):
        ...
        keys = torch.cat(
            [
                self.null_address.repeat(batch_size, 1, 1),
                keys,
                self.new_address.repeat(batch_size, 1, 1),
            ],
            dim=1,
        )
        ...
        mask = torch.zeros(
            (batch_size, num_entities + 2), device=scores.device, dtype=scores.dtype
        )
...
class MemoryModule(nn.Module):
    def __init__(self, config):
        ...
        self.context_attn = nn.MultiheadAttention(
            self.hidden_size, config["context_attn_heads"], batch_first=True
        )
    ...
    def update(self, context, query_span, entity_memory, scores):
        ...
        scores = scores[
            :, 1:
        ]  # [batch_size, num_entities + 1] here we remove the null score.
        ...
        prev_entity_memory = torch.cat(
            [entity_memory, query_span], dim=1
        )  # [batch_size, num_entities + 1, hidden_size]

        # 1) NTM Read from memory and aggregate information from other entries based on the scores.
        read_out = scores.unsqueeze(2) @ (
            scores.unsqueeze(1) @ prev_entity_memory
        )  # [batch_size, 1 + num_entities, hidden_size]
        ...
        context_attn, _ = self.context_attn(
            self.query_affine(prev_entity_memory), context, context
        )

        # 2) Query span embedding.
        query_span = query_span.repeat(
            1, num_entities + 1, 1
        )  # [batch_size, num_entities + 1, hidden_size]

        focus_memory = torch.cat(
            [read_out, context_attn, query_span], dim=-1
        )  # [batch_size, num_entities + 1, hidden_size * 3]

        # Reshape to be compatible with GRU cell.
        focus_memory = focus_memory.view((num_entities + 1) * batch_size, -1)
        prev_entity_memory = prev_entity_memory.view(
            (num_entities + 1) * batch_size, -1
        )

        # update the memory
        entity_memory = self.gru_cell(
            focus_memory, prev_entity_memory
        )  # [ (batch_size * num_entities + 1) , hidden_size]

        # Reshape back.
        entity_memory = entity_memory.view(
            batch_size, num_entities + 1, hidden_size
        )  # [batch_size, num_entities + 1, hidden_size]
    ...
    def forward(
        ...
        if entity_memory is None:
            entity_memory = torch.empty(
                batch_size, 0, hidden_size, device=query_span.device
            )
            entity_size = torch.zeros(
                batch_size, device=query_span.device, dtype=torch.long
            )
            num_entities = 0
        else:
        ...
        predict_scores = self.ntm_addresser(
            query_span, entity_memory, entity_size, mention_score
        )  # [batch_size, 2 + num_entities]
        ...
        new_entity_memory = torch.zeros(
            batch_size,
            num_entities + 1,
            self.hidden_size,
            device=entity_memory.device,
            dtype=entity_memory.dtype,
        )

        # Update the memory with the new information from the query_span embedding based on the scores.
        if len(to_be_updated) > 0:
            updated_memory = self.update(
                context[to_be_updated],
                query_span[to_be_updated],
                entity_memory[to_be_updated],
                _scores[to_be_updated],
            )

            new_entity_memory[to_be_updated] = updated_memory
            new_entity_memory[
                to_be_updated, entity_size[to_be_updated]
            ] = updated_memory[
                :, -1
            ]  # Since the new entry is always the last one, regardless of the entity size.

            new_entry_exists = best_cluster_idx[
                to_be_updated
            ] == new_entity_memory.size(
                1
            )  # If a new entry is there increase the entity count.
            new_entity_size[to_be_updated] += new_entry_exists

            best_cluster_idx[
                best_cluster_idx == new_entity_memory.size(1)
            ] = new_entity_size[best_cluster_idx == new_entity_memory.size(1)]

        # Keep the memory unchanged where the best cluster idx == 0. i.e. not a mention.
        if len(not_a_mention) > 0:
            new_entity_memory[not_a_mention] = torch.cat(
                [
                    entity_memory[not_a_mention],
                    torch.zeros(
                        len(not_a_mention),
                        1,
                        hidden_size,
...
if __name__ == "__main__":
    ...
    opt = torch.optim.SGD(
        [{"params": model.parameters()}, {"params": span_encoder.parameters()}], lr=0.05
    )
    ...
    for i in range(1, 400):
        ...
        for t in range(spns.size(1)):  # this is top k spans
            t_spans = [
                tuple(spn.tolist()) for spn in spns[:, t]
            ]  # spans in current step.
