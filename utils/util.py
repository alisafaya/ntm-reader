...
def load_params(module, log_path, key):
	...
	try:
		...
		if missing or unexpected:
			print(f"Did not find (using defaults):{str(missing)}...\n\n" +
...
def make_evict_function(params):
	...
	if params["name"] == "singletons":
		...
		return lambda cluster, idx: (len(cluster) == 1 and idx - cluster.start > params["distance"] or idx - cluster.start > 2 * params["distance"])
	else:
		return lambda cluster, idx: False
