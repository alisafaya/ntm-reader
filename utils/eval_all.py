...
import json
import numpy as np
from utils.metrics import CorefEvaluator, Evaluator
...
def update_evaluators(evaluators, document, predicted_clusters, gold_clusters):
    (
        ...
        mention_to_gold,
    ) = util.mention_maps(predicted_clusters, gold_clusters)

    def keyed_update(key):
        evaluators[key].update(
...
def print_cluster(doc, clusters):
    ...
    for i, cluster in enumerate(clusters):
        if len(cluster) > 1:
            ...
            for span in cluster:
                print(
                    f"{' '.join(tokens[span[0]:span[1] + 1])} [{span[0]}, {span[1] + 1}]"
                )
            print("-" * 80)
    ...
    for cluster in clusters:
        ...
        for i, span in enumerate(spans[1:]):
            if segment_map[prev_span[0]] != segment_map[span[0]]:
                ...
                relaxed_seam_spans[tuple(span)] = spans[
                    : i + 1
                ]  # any antecedent is okay
            prev_span = span
...
def seam_evaluation(exp):
    ...
    for key, document in exp.items():
        ...
        relaxed_intersection = [
            (ant, span)
            for ant, span in predicted_crossings
...
def renumber(clusters, sentences):
    ...
    def fix_span(span):
        ...
        return [span[0] - num_fillers, span[1] - num_fillers]
...
def distance_eval(exp):
    ...
    for _, document in exp.items():
        gold_clusters = renumber(
            document["clusters"], document["sentences"]
        )  # [CLS] and [SEP] get in the way
        predicted_clusters = renumber(
            document["predicted_clusters"], document["sentences"]
        )
        gold_spread.extend(
            [calc_spread(cluster) for cluster in gold_clusters if len(cluster) > 1]
        )
        predicted_spread.extend(
            [calc_spread(cluster) for cluster in predicted_clusters if len(cluster) > 1]
        )
...
def evaluate_exp(exp, simple=False):
    ...
    if simple:
        print(
            f"p/r/avg f1: {evaluators[ALL].prf_str()}, ({evaluators[ALL].get_count()} docs) [{evaluators[ALL].evaluators[3].get_f1()} ment f1]"
        )
    else:
        eval_dict = {
            key: f"{key}: {evaluator.prf_str()}, ({evaluator.get_count()} docs)"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--all_metrics",
        action="store_true",
        help="Print intermediate metrics on subsplits.",
    )
    parser.add_argument(
        "-m", "--mean", action="store_true", help="Report mean and std of all runs"
    )
