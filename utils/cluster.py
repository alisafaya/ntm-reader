...
class SpanLike:
    ...
    def bracket_string(self):
        return self.sentence[
            self.start - self.sentence_offset : self.end - self.sentence_offset + 1
        ]
...
class Cluster(SpanLike):
    def __init__(self, span, debug_embs):
        super(Cluster, self).__init__(
            span.start, span.end, span.sentence, span.sentence_offset
        )
    ...
    def merge(self, cluster, score=0.0):
        self.emb, metadata = cluster, score
        ...
        if cluster.first.start < self.first.start or (
            cluster.first.start == self.first.start
            and cluster.first.end < self.first.end
        ):
            self.first = SpanLike(
                cluster.first.start,
                cluster.first.end,
                cluster.first.sentence,
                cluster.first.sentence_offset,
            )
        self.scores.extend(cluster.scores)
        self.spans.extend(cluster.spans)
        self.score = (self.size * self.score + cluster.size * cluster.score) / (
    ...
    def reset(self):
        ...
        self.spans = (
            []
        )  # Pick first one as canonical mention? Or maybe it should be max
        self.scores = []
        self.merge_data = [1.0]
        self.probs = [1.0]
...
class ClusterList(list):
  ...
  """
    ...
    """
        ...
        self.restrict_clusters(clusters)
        self.detach_()
        for cluster in self.clusters:
    ...
    def restrict_clusters(self, clusters, first_only=False):
        ...
        for cluster in clusters:
            if first_only:
                span_to_cluster[(cluster.first.start, cluster.first.end)] = (
                    num_clusters + 1
                )
            else:
                curr_span_idx = self.span_to_cluster[
                    (cluster.first.start, cluster.first.end)
                ]
    ...
    def get_cluster_id(self, span, original=0):
        ...
        return self.span_to_cluster.get(span, self.span_to_cluster.get(original, 0))
    ...
    def get_clusters(
        self, singleton_eval, condensed=False, print_clusters=True,
    ):
        ...
        sorted_total = self.finalize_clusters(
            lambda x: len(x) > (0 if singleton_eval else 1)
        )
        response_dict = {}
        extracted_clusters = []
        cluster_embs = []
        ...
        for cluster in sorted_total:
            ...
            for span in cluster.spans:
                if not condensed:
                    ...
                    maybe_print(
                        "{}, {}: {}".format(span.start, span.end, span.bracket_string())
                    )
    ...
    def ugly_print(self, limit=1000):
        ...
        print(
            "\n\n".join(
                [
                    "\t\t".join([" ".join(s.bracket_string()) for s in c.spans])
    ...
    def as_list(self, singleton_eval):
        return [
            cluster.as_list()
            for cluster in self.finalize_clusters(
                lambda x: len(x) > (0 if singleton_eval else 1)
            )
        ]
